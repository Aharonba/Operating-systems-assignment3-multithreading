...
int Graph_deque::getNumVertices()
{
    return V;
}

// Helper function to perform DFS and count nodes in an SCC
void Graph_deque::DFSCount(int s, bool visitedV[], int &count)
{
    visitedV[s] = true;
    count++; // Increment the SCC size count for each visited vertex

    // Visit all adjacent vertices
    for (auto i = adj[s].begin(); i != adj[s].end(); ++i)
        if (!visitedV[*i])
            DFSCount(*i, visitedV, count);
}

// Function to return the size of the largest SCC after running Kosaraju's algorithm

int Graph_deque::getLargestSCCSize()
{
    stack<int> Stack;
    ...
    int largestSCCSize = 0; // Track the largest SCC size
    ...
    while (!Stack.empty())
    {
        ...
        if (!visitedV[s])
        {
            ...
            gr.DFSCount(s, visitedV, currentSCCSize); // Count nodes in the current SCC

            // Update largest SCC size if the current one is larger
            if (currentSCCSize > largestSCCSize)
            {
                largestSCCSize = currentSCCSize;
            }
        }
    }

    delete[] visitedV; // Free allocated memory for visited array
...
// Function to remove an edge from vertex s to vertex d
void Graph_deque::removeEdge(int s, int d)
{
    adj[s].erase(std::remove(adj[s].begin(), adj[s].end(), d), adj[s].end()); // Remove edge using std::remove
}

// Function to clear the graph by deleting the adjacency list
